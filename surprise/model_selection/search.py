...
from ..utils import get_rng, sp_version
...
class RandomizedSearchCV:
    ...
    def __init__(self, algo_class, param_distributions, n_iter=10,
        ...
        self.algo_class = algo_class
        self.param_distributions = param_distributions.copy()
        self.n_iter = n_iter
        self.measures = [measure.lower() for measure in measures]
        self.cv = cv
        if isinstance(refit, string_types):
        ...
        # As sim_options and bsl_options are dictionaries, they require a
        # special treatment.
        if 'sim_options' in self.param_distributions:
            sim_options = self.param_distributions['sim_options']
            sim_options_list = [dict(zip(sim_options, v)) for v in
                                product(*sim_options.values())]
            self.param_distributions['sim_options'] = sim_options_list

        if 'bsl_options' in self.param_distributions:
            bsl_options = self.param_distributions['bsl_options']
            bsl_options_list = [dict(zip(bsl_options, v)) for v in
                                product(*bsl_options.values())]
            self.param_distributions['bsl_options'] = bsl_options_list
    ...
    def fit(self, data):
        ...
        sampled parameter combinations, over different splits given by the
        ``cv`` parameter.
