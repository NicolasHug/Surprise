...
def AUC(pred_testset, pred_anti_testset, trainset):
    ...
    for u, j, _, hat_x_uj, _ in pred_anti_testset:
        pred_u_anti_testset[u].append(hat_x_uj)

    print(trainset.n_users)
    print(len(pred_testset))

    AUC = 0
    for u, _, _, hat_x_ui, _ in pred_testset:
        AUC_u = np.mean([(hat_x_ui > hat_x_uj) for hat_x_uj in
                         pred_u_anti_testset[u]])
        AUC += AUC_u
...
def test_zob():
    ...
    mf = Dumb()
    loo = LeaveOneOut(min_n_ratings=0)
