...
def test_mae():
    ...
    assert pyrec.stats.mae(predictions) == 0

    predictions = [pred(0, 0), pred(0, 2)]
    assert pyrec.stats.mae(predictions) == abs(0 - 2) / 2

    predictions = [pred(2, 0), pred(3, 4)]
    assert pyrec.stats.mae(predictions) == (abs(2 - 0) + abs(3 - 4)) / 2

def test_rmse():

    predictions = [pred(0, 0), pred(1, 1), pred(2, 2), pred(100, 100)]
    assert pyrec.stats.rmse(predictions) == 0

    predictions = [pred(0, 0), pred(0, 2)]
    assert pyrec.stats.rmse(predictions) == sqrt((0 - 2)**2 / 2)

    predictions = [pred(2, 0), pred(3, 4)]
    assert pyrec.stats.rmse(predictions) == sqrt(((2 - 0)**2 + (3 - 4)**2) / 2)

def test_fcp():

    predictions = [pred(0, 0, u0='u1'), pred(1, 1, u0='u1'), pred(2, 2,
                   u0='u2'), pred(100, 100, u0='u2')]
    assert pyrec.stats.fcp(predictions) == 1

    predictions = [pred(0, 0, u0='u1'), pred(0, 0, u0='u1')]
    with pytest.raises(ValueError):
        pyrec.stats.fcp(predictions)
    ...
    assert pyrec.stats.fcp(predictions) == 0
